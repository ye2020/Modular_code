...
static uint8_t Usart6_Rx[USART6_RX_LEN] = {0};
static uint8_t Usart6_Tx[USART6_TX_LEN] = {0};
static uint8_t Usart6_Rx_Buffer[USART6_RX_LEN] = {0};
static uint8_t Usart6_Tx_Buffer[USART6_TX_LEN] = {0};

fifo_rx_def fifo_usart6_rx;
fifo_rx_def fifo_usart6_tx;

fifo_rx_def *pfifo_referee = &fifo_usart6_rx;
...
//    __HAL_DMA_ENABLE_IT(huart2.hdmatx, DMA_IT_TC);
    ...
    /* 使能串口DMA接收 */
		HAL_UART_Receive_DMA(&huart6, Usart6_Rx, USART6_RX_LEN); // 开启串口DMA接收（记得开启循环模式）
    if (fifo_init(pfifo_referee, Usart6_Rx_Buffer, USART6_RX_LEN) == -1)
    {
        Error_Handler(); // 必须 2 的幂次方
    }

    /* 使能串口DMA发送 */
    __HAL_DMA_DISABLE(huart6.hdmatx);  // 关闭串口DMA发送通道 （不用开启循环模式）
    if (fifo_init(pfifo_referee, Usart6_Tx_Buffer, USART6_TX_LEN) == -1)
    {
        Error_Handler(); // 必须 2 的幂次方
    }
...
void USER_UART6_IRQHandler(UART_HandleTypeDef *huart)
{
...
//    __HAL_DMA_CLEAR_FLAG(huart ->hdmarx, DMA_FLAG_TCIF1_5);      // 清除传输完成标准位 
		__HAL_UART_CLEAR_PEFLAG(huart);															 // 清除传输完成标志位 

		__HAL_DMA_DISABLE(huart ->hdmarx);
//    __HAL_DMA_DISABLE(&hdma_usart3_rx);                            // 关闭串口DMA发送通道
		HAL_UART_Receive_DMA(huart, Usart6_Rx, USART6_RX_LEN); // 开启串口DMA接收（记得开启循环模式）

    num = USART6_RX_LEN - __HAL_DMA_GET_COUNTER(huart ->hdmarx);  //! 获取DMA中未传输的数据个数，NDTR寄存器分析参考中文参考手册 （DMA_Channel_TypeDef）  这个不同的芯片HAL库里面定义的命名有点不同

    fifo_write_buff(pfifo_referee ,Usart6_Rx,num);                // 写入环形队列
		
		__HAL_DMA_ENABLE(huart ->hdmarx);														// 开启DMA（记得呀 ！！）
    HAL_UART_Receive_DMA(huart,Usart6_Rx,USART6_RX_LEN);       // 启动DMA
  }
...
uint8_t Judge_Data_TF = 0; //裁判数据是否可用,辅助函数调用
